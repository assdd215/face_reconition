...
def compare():
    imgs_data = [pic1,pic2]
    result = predeal.load_img(imgs_data)
...
def main(test_path = "imgs/322.jpg",top_n = 100,use_weight = False):

    if os.path.exists(database_file) == False:
        print("当前没有数据文件，先进行制作")
        embedDatabaseAndSaveAsNpy()
    predeal.check_all_in_database(test_path)
    ...
    if use_weight:  # 为结果加上时间权重
        all_result = sort_result_with_weight(all_result)
...
def embedDatabaseAndSaveAsNpy():
    ...
    imgs_dict = predeal.load_imgs(use_to_save=False)
    img_index,img_content = changeDictToArray(imgs_dict)
    print("img loaded")
    print("start embedding...")
    img_emb = predeal.embedPic(img_content)
    print("img emded")
    print("start saving...")
...
'''
参照了 WT(u,i) = (1-a) + a* D / L 计算权重公式
a 指调节权重的值，
L 指我们指定的某个时间点阈值，可以理解为最大长度
D 指item记录的时间点到阈值L的时间间隔
方法未进行测试
'''
def sort_result_with_weight(result):
    a = 0.5
    L = 60
    for item in result:
        D = item.time - L
        W = ( 1 - a ) + a * D / L
        item.value = W * item.value
    result.sort(cmp=cmp)
    return result
